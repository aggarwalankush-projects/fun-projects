...
public class GenerateTagsTwoA {

	String firstModel = "firstModel.bin";
	String testFile = "testing.test";
	String predictions = "part-II-predictions.tsv";

	HashSet<String> snn = new HashSet<String>();
	HashSet<String> svb = new HashSet<String>();
	HashSet<String> sjj = new HashSet<String>();
	HashSet<String> srb = new HashSet<String>();

	public GenerateTagsTwoA() {
		snn.add("NN");
		snn.add("NNS");
		snn.add("NNP");
		snn.add("NNPS");
	...
	public String doMapping(String mapMe) {
		if (snn.contains(mapMe))
			return "SNN";
		else if (svb.contains(mapMe))
			return "SVB";
		else if (sjj.contains(mapMe))
			return "SJJ";
		else if (srb.contains(mapMe))
			return "SRB";
		else
			return "misc";
	}
	...
	public void doAllWork() throws Exception {
		...
		while ((line = br.readLine()) != null) {
			...
			for (String s : sent) {
				...
				newTag.append(doMapping(tt[tt.length - 1]) + " ");

				oldTag.append(doMapping(words[1]) + " ");
				newLine.append(words[0] + "/" + doMapping(words[1]) + " ");
				for (int i = 0; i < tt.length - 1; i++)
					tagline.append(tt[i] + "/");
				tagline.append(doMapping(tt[tt.length - 1]) + " ");
			}
			bw.write(id + "\t" + newLine + "\t" + tagline + "\n");
...
//		System.out.println("\n" + ir);
	...
	public static void main(String[] args) {
		GenerateTagsTwoA generateTags = new GenerateTagsTwoA();
		try {
